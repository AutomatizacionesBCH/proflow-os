...
import { createEmpresa, updateEmpresa, type EmpresaInput } from '@/app/empresas/actions'

const ALL_STATUSES: { value: EmpresaStatus; label: string }[] = [
...
function initialValues(editing?: Company): FormValues {
  if (editing) {
    return {
      name:       editing.name,
  ...
  return { name: '', legal_name: '', status: 'activo', notes: '' }
}

export function EmpresaForm({ onClose, onSuccess, editing }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const input: EmpresaInput = { ...form }

    startTransition(async () => {
      const result = editing
        ? await updateEmpresa(editing.id, input)
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-full max-w-md bg-slate-900 border-l border-slate-800 flex flex-col h-full shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              {editing ? `Modificar datos de ${editing.name}` : 'Registrar entidad legal'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
          ...
          <Field title="Razón social / Nombre legal">
            <input
              className={inputCls}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-800 flex-shrink-0">
          ...
          >
            {isPending ? 'Guardando…' : editing ? 'Guardar cambios' : 'Crear empresa'}
          </button>
        </div>
      </div>
    </div>
